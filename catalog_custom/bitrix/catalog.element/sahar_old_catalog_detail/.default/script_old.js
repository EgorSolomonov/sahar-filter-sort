...
window.JCCatalogElement.prototype.UpdateRow = function(intNumber, activeID, showID, canBuyID)
{
	...
	if (-1 < intNumber && intNumber < this.obTreeRows.length)
	{
		...
		if (!!RowItems && 0 < RowItems.length)
		{
			...
			{
				BX.adjust(this.obTreeRows[intNumber].CONT, {props: {className: (extShowMode < countShow ? 'bx_item_detail_size' : 'bx_item_detail_size')}});
			}
			if (extShowMode)
			{
			...
			if (!!this.sliders[i].ID)
			{
				if (i === index)
				{
					this.sliders[i].START = 0;
                                        //BX.adjust(this.sliders[i].CONT, {style: { visibility: 'visible'}});
					BX.adjust(this.sliders[i].CONT, {style: { display: ''}});
                                        
                                        var owl = $(this.sliders[i].LIST).data('owlCarousel');
                                        if (owl && typeof owl.onResize === "function") {
                                            owl.onResize();
                                        }
				}
				else
				{
					//BX.adjust(this.sliders[i].CONT, {style: { visibility: 'hidden'}});
		...
		if (price.DISCOUNT_VALUE !== price.VALUE)
		{
			if (this.config.showOldPrice)
			{
				...
				if (!!this.obPrice.discount)
				{
					...
					BX.adjust(this.obPrice.discount, {style: {display: 'none'}, html: economyInfo });
				}
			}
			if (this.config.showPercent)
			{
