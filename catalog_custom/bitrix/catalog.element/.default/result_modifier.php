...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer) {
		...
		$AVAILABILITY = '';
		if (isset($arOffer['PROPERTIES']['AVAILABILITY']['VALUE'])) {
			$AVAILABILITY = $arOffer['PROPERTIES']['AVAILABILITY']['VALUE'];
		}
...
if (count($arResult["OFFERS"]) > 0) {
	if ($arParams["CURRENT_OFFER"] > 0) {
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer) {
			if ($arOffer["ID"] == $arParams["CURRENT_OFFER"]) {
				$arResult["OFFERS_SELECTED"] = $keyOffer;
			}
		}
	}
}
...
if (CModule::IncludeModule("sale") && CModule::IncludeModule("catalog")) {
	$elements = [];
	$arSelect = array("ID", "PREVIEW_PICTURE", "DETAIL_PAGE_URL", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM", "PROPERTY_*");
	...
	while ($ob = $res->GetNextElement()) {
		...
		if ($priceArray = $priceRes->Fetch()) {
			$arFields["PRICE"] = round($priceArray["PRICE"]);
			$arFields["CURRENCY"] = $priceArray["CURRENCY"];
		}
		...
		$orderRes = CSaleOrder::GetList(
			...
			array(">=DATE_INSERT" => ConvertTimeStamp(strtotime("-1 month"), "FULL")), // За последний месяц
			false,
			false,
			array("ID")
		);
		...
		while ($order = $orderRes->Fetch()) {
			...
			while ($basket = $basketRes->Fetch()) {
				if (!isset($salesData[$basket["PRODUCT_ID"]])) {
					$salesData[$basket["PRODUCT_ID"]] = 0;
				}
				$salesData[$basket["PRODUCT_ID"]] += $basket["QUANTITY"];
			}
		}

		// Сортируем товары по количеству продаж
		usort($elements, function ($a, $b) use ($salesData) {
			$salesA = isset($salesData[$a["ID"]]) ? $salesData[$a["ID"]] : 0;
			$salesB = isset($salesData[$b["ID"]]) ? $salesData[$b["ID"]] : 0;
