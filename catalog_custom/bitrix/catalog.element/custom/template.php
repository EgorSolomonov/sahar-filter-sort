	...
	if (!empty($arResult['SKU_PROPS'])) {
		foreach ($arResult['SKU_PROPS'] as $arProp) {
			...
			if ('TEXT' == $arProp['SHOW_MODE']) {
				...
				$skuTemplate[$propId]['FULL']['START'] = '<span class="scu_name">' . htmlspecialcharsbx($arProp['NAME']) . ':</span><ul>';
				$skuTemplate[$propId]['FULL']['FINISH'] = '</ul>';
				foreach ($arProp['VALUES'] as $value) {
					$value['NAME'] = htmlspecialcharsbx($value['NAME']);
					$skuTemplate[$propId]['ITEMS'][$value['ID']] = '<li>' . $value['NAME'] . '</li>';
				}
				unset($value);
...
<div class="bx_catalog_list_home">
	<? // Спец символ для ленивой загрузки
	?>
	<!--RestartBuffer-->
	<?
	...
	?>
		<div class="bx_catalog_item"><a class="bx_catalog_item_container" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" id="<? echo $strMainID; ?>">
				<div id="<? echo $arItemIDs['PICT']; ?>" class="bx_catalog_item_images" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>')">
					<div class="stickers">
						<? if ($arItem['PROPERTIES']["NOVINKA"]["VALUE"] == 'Да') : ?>
							<div class="sticker_novinka"></div>
						<? endif; ?>
						<? if ($arItem['PROPERTIES']["LIDER_PRODAZH"]["VALUE"] == 'Да') : ?>
					...
					<div class="labels_wrp">
						...
						<!--			<div id="--><? // echo $arItemIDs['STICKER_ID']; 
													?><!--" class="bx_stick average left top" title="--><? // echo $arItem['LABEL_VALUE']; 
																										?><!--">--><? // echo $arItem['LABEL_VALUE']; 
																													?><!--</div>-->
						<!--	--><? //
									//	}
									?>
						<? if ($arItem["PROPERTIES"]["HIT"]["VALUE_XML_ID"] == "Y") : ?>
							<div class="label label_hit"><?= GetMessage("HIT"); ?></div>
						<? elseif ($arItem["PROPERTIES"]["NEW"]["VALUE_XML_ID"] == "Y") : ?>
							<div class="label label_new"><?= GetMessage("NEW"); ?></div>
						<? endif; ?>

					</div>

					<div class="quick-view-btn btn" data-href="<?= $arItem["DETAIL_PAGE_URL"] ?>" data-id="<?= $arItem["ID"] ?>"><?= GetMessage("QUICK_VIEW"); ?></div>
				</div>
				<div class="bx_catalog_item_info">
					<?
					if ($USER->IsAdmin()) { ?>
						<p style="font-size:11px; margin-bottom:0px;">Новый год - <?= $arItem['PROPERTIES']['NOVYY_GOD']['VALUE'] ?></p>
						<!--                                        <p style="font-size:11px; margin-bottom:0px;">Зима - <?= $arItem['PROPERTIES']['SEZON_ZIMA']['VALUE'] ?></p>-->
						<p style="font-size:11px; margin-bottom:0px;">Осень-зима - <?= $arItem['PROPERTIES']['OSEN_ZIMA']['VALUE'] ?></p>
						<p style="font-size:11px; margin-bottom:0px;">Весна лето - <?= $arItem['PROPERTIES']['VESNA_LETO']['VALUE'] ?></p>
						<!--                                        <p style="font-size:11px; margin-bottom:0px;">Лето - <?= $arItem['PROPERTIES']['SEZON_LETO']['VALUE'] ?></p>-->
					<? } ?>
					<? //dump($arItem['PROPERTIES']['NOVYY_GOD']['VALUE'])
					?>
					<div class="bx_catalog_item_title"><? echo $productTitle; ?>

					</div>
					<div class="bx_catalog_item_price">
						<div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price">
							<? if (($arItem['PROPERTIES']["OLD_PRICE"]["VALUE"]) && ($arItem['MIN_PRICE']['VALUE'] != $arItem['PROPERTIES']["OLD_PRICE"]["VALUE"])) : ?>
								<? //dump($arItem['MIN_PRICE']['VALUE'])
								?>
								<span class="old_price"> <?= $arItem['PROPERTIES']["OLD_PRICE"]["VALUE"] ?></span>
							<? endif; ?>
							<?
							if (!empty($minPrice)) {
							...
							?>
									<span class="old_price"><? //=$str
															?> <? echo $minPrice['PRINT_VALUE']; ?></span>
								<?
								}
								if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
								?><span>
				...
				?><script type="text/javascript">
					...
					<?
								...
								if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'] || 'N' == $arParams['PRODUCT_DISPLAY_MODE']) {
																	...
																	foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer) {
																		...
																		if (!empty($arJSOffer['DISPLAY_PROPERTIES'])) {
																			foreach ($arJSOffer['DISPLAY_PROPERTIES'] as $arOneProp) {
																				$strProps .= '<br>' . $arOneProp['NAME'] . ' <strong>' . (is_array($arOneProp['VALUE'])
	...
	if ($paramValue < $pageCount) {
		...
		echo sprintf(
			'<div class="ajax-pager-wrap" >
                      <a class="ajax-pager-link" data-wrapper-class="bx_catalog_list_home" href="%s"></a>
                  </div>',
			$url
		);
	}
	// LazyLoad ссылка конец
	?>
	<? // Спец символ для ленивой загрузки
	?>
	<!--RestartBuffer-->
